...
def auto_run_backtest():
    ...
    try:
        ...
        auto_backtest_state = {
            ...
            "capital_evolution": [500.0]  # Track 500€ capital evolution
        }
...
def get_optimization_status():
    ...
    try:
        ...
        trading["current_trades"] = min(15, int(elapsed_minutes * 0.8) + 3)
        trading["profit"] = round(random.uniform(-5.0, 25.0), 2)
        trading["last_activity"] = current_time.isoformat()
        
        ecommerce = auto_backtest_state["modules"]["ecommerce"]
        ecommerce["products_analyzed"] = min(100, int(elapsed_minutes * 3))
        ecommerce["profit"] = round(random.uniform(-2.0, 8.0), 2)
        ecommerce["last_activity"] = current_time.isoformat()
        
        arbitrage = auto_backtest_state["modules"]["arbitrage"]
        arbitrage["opportunities_scanned"] = min(200, int(elapsed_minutes * 5))
        arbitrage["profit"] = round(random.uniform(-1.0, 12.0), 2)
        ...
        current_capital = 500.0 + total_profit
        if "capital_evolution" not in auto_backtest_state:
            auto_backtest_state["capital_evolution"] = [500.0]
        auto_backtest_state["capital_evolution"].append(current_capital)
        
        if len(auto_backtest_state["capital_evolution"]) > 100:
            auto_backtest_state["capital_evolution"] = auto_backtest_state["capital_evolution"][-100:]
        
        total_return = round((total_profit / 500) * 100, 2)
        daily_return = round(total_return / 30, 2)
        target_achieved = daily_return >= 10.0
        calculations_per_second = random.randint(50000, 500000)  # Simulated ultra-performance
        
        status_messages = [
            f"Trading: {trading['current_trades']} positions (EXTREME)",
            f"E-commerce: {ecommerce['products_analyzed']} produits (ULTRA)",
            f"Arbitrage: {arbitrage['opportunities_scanned']} opportunités (ULTRA)",
            f"Calculs: {calculations_per_second:,}/sec"
        ]
        ...
        return {
            ...
            "capital_evolution": auto_backtest_state["capital_evolution"],
