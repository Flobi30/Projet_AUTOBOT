...
def test_trend_shadow_lab_persists_entry_features_on_closed_trade(tmp_path):
    lab = _lab(tmp_path)
    prices = [100.0, 101.0, 102.0, 104.0, 108.0, 110.0, 107.0]

    for idx, price in enumerate(prices):
        lab.on_price_tick(symbol="FEATUREEUR", price=price, timestamp=f"2026-05-20T00:4{idx}:00+00:00")
    lab.flush()
