...
def _render_autonomous_review_text(report: Dict[str, Any]) -> str:
    ...
    lines.append(f"Generated at: {report.get('generated_at')}")
    lines.append(f"Window hours: {report.get('window_hours')}")
    lines.append(f"Global health: {report.get('global_system_health')}")
    lines.append(f"Recommended action: {report.get('recommended_action')}")
    lines.append(f"Confidence: {report.get('confidence_level')}")
    lines.append("")
    lines.append("Dominant rejection reasons:")
    for item in report.get("dominant_rejection_reasons", []):
        lines.append(f"- {item.get('reason')}: {item.get('count')}")
    lines.append("")
    lines.append("Recommended focus points:")
    for point in report.get("recommended_focus_points", []):
